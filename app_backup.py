...
def mood_tracking():
    ...
    recent_moods = conn.execute(
        'SELECT * FROM mood_entries WHERE user_id = ? ORDER BY timestamp DESC LIMIT 30',
        (user_id,)
    ).fetchall()
    conn.close()
...
def mental_health_insights():
    ...
    mood_avg = conn.execute(
        'SELECT AVG(mood_score) as avg_mood FROM mood_entries WHERE user_id = ? AND timestamp >= date("now", "-30 days")',
        (user_id,)
    ).fetchone()
    
    # Mood trend data for graph
    mood_trends = conn.execute(
        'SELECT DATE(timestamp) as date, AVG(mood_score) as avg_mood FROM mood_entries WHERE user_id = ? AND timestamp >= date("now", "-30 days") GROUP BY DATE(timestamp) ORDER BY date',
        (user_id,)
    ).fetchall()
    
    # Most common mood tags
    all_moods = conn.execute(
        'SELECT mood_tags FROM mood_entries WHERE user_id = ? AND timestamp >= date("now", "-30 days")',
        (user_id,)
    ).fetchall()
    ...
    # Process mood tags
    tag_frequency = {}
    for mood_row in all_moods:
        if mood_row['mood_tags']:
            tags = json.loads(mood_row['mood_tags'])
            for tag in tags:
                tag_frequency[tag] = tag_frequency.get(tag, 0) + 1
    
    insights = {
        "avg_mood": round(mood_avg['avg_mood'], 1) if mood_avg['avg_mood'] else None,
        "mood_trends": [{"date": row['date'], "mood": row['avg_mood']} for row in mood_trends],
        "common_moods": sorted(tag_frequency.items(), key=lambda x: x[1], reverse=True)[:5]
    }
    
    return render_template('mental_health_insights.html', insights=insights)
...
def chat():
    ...
    therapeutic_response = get_personalized_therapeutic_response(sentiment_data, original_user_input)
    
    # Save mood tracking for registered users
    if "user_id" in session:
        save_mood_tracking(session["user_id"], sentiment_data, original_user_input)

    def _translate_response_content(text):
        return _translate_response_to_original_lang(text, original_lang)

    # Initialize response variable
    final_response = None

    # --- Smart Crisis Detection with Personalized Therapeutic Response ---
    if therapeutic_response and (sentiment_data['crisis_detected'] or therapeutic_response['response_type'] == 'crisis_intervention'):
        # Use our enhanced therapeutic response system for CRISIS ONLY
        formatted_therapeutic_response = format_ai_response(therapeutic_response['message'])
        final_response = formatted_therapeutic_response
        
        # Save to database if user is logged in  
        if "user_id" in session:
            save_chat_message(session["user_id"], original_user_input, final_response)
        
        return jsonify({"response": final_response})

    # --- For non-crisis situations, use concise Gemini AI responses ---
    try:
        app.logger.info("Using Gemini AI for concise response generation")
...
def sentiment_insights():
    """Show sentiment analysis and emotional patterns for the user using lightweight system"""
    if "user_id" not in session:
        flash("Please login to view your emotional insights", "error")
        return redirect(url_for("login"))
    
    user_id = session.get("user_id")
    
    # Get recent sentiment data from our lightweight system
    conn = get_db_connection()
    recent_sentiments = conn.execute('''
        SELECT sentiment, confidence_score, emotions_detected, crisis_flag, timestamp
    ...
    # Calculate sentiment statistics for the past 7 days
    ...
    # Get daily mood progression
    daily_moods = conn.execute('''
        SELECT 
        ...
        FROM sentiment_tracking 
        WHERE user_id = ? AND timestamp >= datetime('now', '-14 days')
        GROUP BY DATE(timestamp), sentiment
        ORDER BY date DESC
    ''', (user_id,)).fetchall()
    
    # Get emotion frequency analysis 
    emotion_analysis = conn.execute('''
        SELECT 
            emotions_detected,
            COUNT(*) as frequency,
            sentiment,
            timestamp
        FROM sentiment_tracking 
        WHERE user_id = ? AND emotions_detected != '' AND timestamp >= datetime('now', '-7 days')
    ...
    # Process emotions data (since emotions are stored as comma-separated)
    emotion_frequency = {}
    for record in emotion_analysis:
        emotions_detected = record['emotions_detected']
        if emotions_detected and emotions_detected.strip():  # Check for null/empty
            emotions = emotions_detected.split(',')
            for emotion in emotions:
                emotion = emotion.strip()
                if emotion:
                    emotion_frequency[emotion] = emotion_frequency.get(emotion, 0) + record['frequency']
    
    # Prepare data for charts
    sentiment_labels = [stat['sentiment'] for stat in sentiment_stats]
    sentiment_counts = [stat['count'] for stat in sentiment_stats]
    
    # Sentiment to color mapping for visualization
    sentiment_colors = {
        'severe_negative': '#e74c3c',
        'moderate_negative': '#f39c12', 
        'mild_negative': '#f1c40f',
        'neutral': '#95a5a6',
        'mild_positive': '#2ecc71',
        'positive': '#27ae60'
    }
    
    chart_colors = [sentiment_colors.get(label, '#bdc3c7') for label in sentiment_labels]
    
    # Daily trend data
    daily_dates = [mood['date'] for mood in daily_moods]
    daily_sentiment_scores = []
    daily_crisis_alerts = []
    
    # Convert sentiment to numerical score for trend analysis
    sentiment_to_score = {
        'severe_negative': -3,
        'moderate_negative': -2,
        'mild_negative': -1, 
    ...
    for mood in daily_moods:
        score = sentiment_to_score.get(mood['sentiment'], 0)
        daily_sentiment_scores.append(score)
        daily_crisis_alerts.append(mood.get('crisis_count', 0))
    
    # Recent insights and recommendations
    insights = []
    if recent_sentiments:
        latest_sentiment = recent_sentiments[0]['sentiment']
        
        if latest_sentiment in ['severe_negative', 'moderate_negative']:
            insights.append({
                'type': 'concern',
                'message': 'Your recent interactions show signs of distress. Consider reaching out to mental health professionals.',
                'icon': '⚠️'
            })
        elif latest_sentiment == 'positive':
            insights.append({
                'type': 'positive',
                'message': 'Great to see you\'re feeling positive! Keep up the good mental health practices.',
                'icon': '🌟'
            })
        
        # Crisis frequency check
        crisis_count = sum(1 for s in recent_sentiments[:10] if s['crisis_flag'])
        if crisis_count > 0:
            insights.append({
                'type': 'crisis',
                'message': f'Crisis indicators detected {crisis_count} times in recent conversations. Please consider immediate professional support.',
                'icon': '🚨'
            })
    
    # Top emotions insight
    if emotion_frequency and len(emotion_frequency) > 0:
        ...
        if top_emotion:
            insights.append({
                'type': 'emotion',
                'message': f'Your most common emotional state recently: {top_emotion.replace("_", " ").title()}',
                'icon': '💭'
            })
    
    # Reverse the daily data for chronological order (oldest to newest)
    daily_dates_reversed = list(reversed(daily_dates))
    daily_sentiment_scores_reversed = list(reversed(daily_sentiment_scores))
    daily_crisis_alerts_reversed = list(reversed(daily_crisis_alerts))
    ...
    return render_template('sentiment_insights.html', 
                         ...
                         daily_dates=daily_dates_reversed,
                         daily_sentiment_scores=daily_sentiment_scores_reversed,
                         daily_crisis_alerts=daily_crisis_alerts_reversed,
                         insights=insights)
